...
Standards (from HATCH layer analysis + repo rendering adjustments):
  - Layer HATCH, color 123 (light cyan)
  - 4" walls: ANSI31 pattern, color 256 (bylayer → 123)
  - 6" walls: SOLID,_O pattern, color 9 (gray, entity override)

Operational note:
  - Although the source CAD analysis recorded ANSI31 scale 6.0, this repo
    intentionally uses 0.3 because ezdxf/viewer rendering produced a much too
    sparse 4" wall fill at larger scales. See commits ab3c45f and ec2402b.
"""
...
def add_wall_hatch(msp: Any, doc: Any, pts: list, thickness: float) -> None:
    ...
    4" (< THRESHOLD) → ANSI31, color bylayer (123 cyan), dense operational scale
